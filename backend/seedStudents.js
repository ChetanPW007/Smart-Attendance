...
if (!DB_URI || DB_URI.includes('localhost')) {
  console.error('\n❌ ERROR: Please set your MongoDB Atlas MONGO_URI in backend/.env first!');
  console.error('   It should look like: mongodb+srv://user:password@cluster.mongodb.net/attendanceDb\n');
  process.exit(1);
}
  ...
  try {
    ...
    for (const s of students) {
      ...
      if (!existing) {
        ...
        console.log(`  ✅ Added: ${s.name} (${s.usn})`);
        inserted++;
      } else {
        console.log(`  ⏭️  Skipped (already exists): ${s.usn}`);
        skipped++;
      }
    }

    console.log(`\n🎉 Done! ${inserted} students added, ${skipped} already existed.`);
    process.exit(0);
  } catch (err) {
